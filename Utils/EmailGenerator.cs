...
namespace CatalogoBackend.Utils
{
    public class EmailGenerator
    {
        ...
        public string GenerateConfirmUrl(string rawToken)
        {
            ...
            var confirmUrl = QueryHelpers.AddQueryString($"{baseFront}/password", query); //? http://localhost:4200/password?token=abc-123_XYZ

            return confirmUrl;
        }

        //* Se generan los cuerpos para los correos
        public BodyBuilder GenerateHtmlBody(string username, string confirmUrl = "", string password = "")
        {
                    ...
                    builder.HtmlBody = $@"
                        <html>
                            <body style='font-family: Arial, sans-serif;'>
                                ...
                                <p>Hola {System.Net.WebUtility.HtmlEncode(username)} tu contraseña se ha establecido y ahora tu cuenta esta activa para poder ser uilizada. Por favor evita mostrar tu contraseña a terceros o infromaci+on personal que pueda poner en peligro la información de cuenta.</p>
                                <hr>
